...
if instance_exists(obj_patience) {
	...
	and (distance_to_object(obj_patience) <= 3) and obj_patience.in_menu == false 
	and obj_patience.in_battle == false and visible == true and obj_patience.in_shop == false) {
		if taken = false { //подходим в первый раз
			...
			if global.items[7].item_name != "" { //проверяем, полный ли инвентарь
				...
				//phrase = pick_up_text + string(self.item_name) + "."; //выводим на экран, что мы нашли предмет
				phrase = pick_up_text; //выводим на экран, что мы нашли предмет
			}
